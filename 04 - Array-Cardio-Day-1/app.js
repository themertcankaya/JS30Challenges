...
const born1500s = inventors.filter(item => {
...
const totalYear = inventors.reduce((acc, cur) => {
...
// const includesDe = [...links].map(item=>item.textContent)
//                             .filter(item => item.includes('de'));

//The above was done by going to the page in the link and using the console of that page.
...
const sortLast = peopleX.sort((a, b) => {
...
const sumEach = data.reduce((acc, cur) => {
    if (!acc[cur]) {
        acc[cur] = 1
    }
    acc[cur]++;
